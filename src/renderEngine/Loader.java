...
public class Loader {
	...
	public RawModel loadToVAO(float[] positions, float[] textureCoords, float[] normals,
			int[] indices, Vector3f maxHeight, Vector3f minHeight, Vector3f middle) {
		int vaoID = createVAO();
		...
		unbindVAO();
		return new RawModel(vaoID, indices.length, maxHeight, minHeight, middle);
	}
	...
	public RawModel loadToVAO(float[] positions, float[] textureCoords, float[] normals, int[] indices, int[] jointIds, float[] weights){
		...
		storeDataInAttributeList(1, 2, textureCoords);
		storeDataInAttributeList(2, 3, normals);
		storeDataInIntAttributeList(3, 3, jointIds);
	...
	public RawModel loadToVAO(MeshData data){
		return loadToVAO(data.getVertices(), data.getTextureCoords(), data.getNormals(), data.getIndices(), data.getJointIds(), data.getVertexWeights());
	}
	...
	public int loadTexture(String fileName) {
		...
		try {
			InputStream textureFile = Class.class.getResourceAsStream("/res/" + fileName + ".png");
			texture = TextureLoader.getTexture("PNG", textureFile);
			textureFile.close();
	...
	private void storeDataInIntAttributeList(int attributeNumber, int coordinateSize, int[] data) {
		int vboID = GL15.glGenBuffers();
		vbos.add(vboID);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboID);
